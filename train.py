...
# @Last Modified time: 2020-12-21 08:17:42
import os
import argparse
import json
import numpy as np
...
def train(model, dataset, params):
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        # This currently streams from disk. You set num_epochs=1 and wrap this call with something like itertools.cycle to keep this data in memory.
        ...
        for step in range(params.num_steps + 1):
            ...
            d_loss_step = update_disc(
                            model,
                            x,
                            model.D_solver, # model.d_opt,
                            model.D_loss,   # model.d_loss,
                            params,
                            session
                        )
            ...
            for i in range(params.num_gen):
                g_loss_i = update_gen(
                                    model,
                                    x,
                                    model.G_solver[i],  # model.g_opt,
                                    model.Gen_loss[i],  # model.g_loss,
                                    model.Al_solver,  
                                    model.Al_gen_loss,  
                                    params,
                                    session)
            ...
            df_gen_losses = df_gen_losses.append({'step': step, 
                                    ...
                                    'g_4_loss': g_loss_list[4]}, ignore_index=True)
...
def main(args):
    ...
    if args.d_dim == 0:
        args.d_dim = args.g_dim     # args.z_dim
